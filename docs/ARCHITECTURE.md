...
- **Fail-safe audit trail.** Per-user deletes run inside a per-user try/catch: a transient failure on one user is recorded in the response `errors[]` and that user's report, and the loop continues, so a live run always returns a complete record of what was and wasn't destroyed. A clean run returns `200`; a run with any scope error returns `207` with the full body.
