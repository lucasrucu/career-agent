...
- **Retention "activity" is writes + sign-ins, not sessions.** The retention job dates a guest by their writes and explicit sign-ins, not by silent session/token refreshes, so a purely read-only guest with a live session can be pruned after 30 days (see §11). It's the right trade for a demo window, but it isn't session-aware.
...
- **Runner** — `app/api/admin/retention/route.ts` (POST, service-role). It lists auth users (paginated via GoTrue's `nextPage` cursor, not a length heuristic, so it never stops early), folds each user's last sign-in and newest row across the five career-agent tables into one activity timestamp, selects the eligible ids, and for each one counts rows per table + storage objects. Every scan that can span many rows is paginated (auth users, the per-table activity read, and storage listing): the activity read pages by the count actually returned and stops only on an empty page, so a shared-project `db-max-rows` cap can't truncate a user's newest row out of the scan and make an active guest look older than they are. It deletes each of `resume_drafts`, `match_results`, `saved_jobs`, `resumes`, `profiles` explicitly (scoped to that `user_id`, not relying on cascade), then removes the user's objects under `resumes/{user_id}/`. Rows are cleared **before** storage so a mid-run failure leaves re-sweepable storage bytes (found next run by `user_id` prefix) rather than orphaned rows pointing at already-deleted files.
- **Guest `access_requests` PII too.** The landing-page "Request Access" form (`0002_access_requests.sql`) stores guest `email`, `ip`, `user_agent`, and `note` with **no** `user_id` — a guest who submits the form but never gets an auth account never enters the per-user scan, so that PII would otherwise persist forever. The runner also prunes `access_requests` rows older than `retentionDays` by `created_at`, scoped to `app = 'career-agent'` so it never touches another app's rows in that shared table.
- **What "activity" means (limitation).** A user's newest activity is the max of auth `created_at`, `last_sign_in_at`, and the newest row across the five tables — i.e. **writes and explicit sign-ins only.** A persistent auto-refreshing session (token refresh with no fresh sign-in and no new writes) does not count, so a read-only guest can still become eligible after 30 days. Accepted for a demo window; add a heartbeat write on read (or read GoTrue refresh times) if it ever matters.
...
- **Two guards, dry-run default.** It refuses without the service-role key, and requires a shared secret in the `x-retention-secret` header (env `RETENTION_SECRET`, compared in constant time) so a random visitor can't trigger it. It only deletes when `RETENTION_DRY_RUN` is exactly `"false"`; otherwise it logs and returns a structured report (user ids, per-table row counts, storage object counts, stale access-request count) and deletes nothing.
- **Fail-safe audit trail (accurate on partial deletes).** Each user's counts are read first (`rows`, `storage_objects`), then the delete runs and reports a per-user `status`: `planned` (dry run, nothing touched), `deleted` (every scope cleared), `partial` (some data destroyed but the run errored mid-way), or `failed` (errored before destroying anything — safe to retry). On a `partial`/`failed` outcome the pre-computed `rows`/`storage_objects` are **preserved, not zeroed**, and a `destroyed: { storage_removed, tables_deleted[] }` object records exactly which scopes actually completed — so the trail never claims "nothing deleted" when data was in fact partially destroyed. The summary carries `partial_users` and `access_requests_deleted` (the true delete flag, distinct from the matched `would_delete_access_requests` count). Classification is the pure `classifyDeleteProgress` in `lib/retention.ts`, unit-tested for the exact tables-gone-then-error case. A clean run returns `200`; any scope error returns `207` with the full body. The `deleted/partial/failed` distinction is verified by the self-test; the live Supabase read/delete calls themselves are covered by build + inspection only.
- **Index migration** — `supabase/migrations/0003_retention_indexes.sql` adds composite `(user_id, timestamp)` indexes for the activity scan and scoped deletes. `access_requests` already has a `created_at` index from `0002`, so no new index is needed for its scan. It is committed but **not applied**.
